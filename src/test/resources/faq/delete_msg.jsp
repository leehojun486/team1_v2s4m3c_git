    ...
    <UL>
        ...
        <c:when test="${param.passwd_cnt == 1 }"> <!-- 패스워드 일치 -->
          <c:choose>
            <c:when test="${param.cnt == 1}"> <!-- 글 삭제 성공 -->
              <LI class='li_none'>
                <span class='span_success'>『${param.faqtitle }』 컨텐츠를 삭제했습니다.</span>
              </LI>
            </c:when>
            <c:otherwise>    <!-- 글 삭제 실패 -->
              <LI class='li_none'>
                <span class='span_fail'>『${param.faqtitle }』 컨텐츠 삭제에 실패했습니다.</span>
              </LI>
              <LI class='li_none'>
      ...
      <c:choose>
        <c:when test="${param.cnt == 1 && param.passwd_cnt == 1}">
          <LI class='li_none'>
            <button type='button' 
                        onclick="location.href='./list_all.do?nowPage=${param.nowPage }'"
